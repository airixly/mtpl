...
/**
 * Pass the absolute path which the module will be generated
 * @param root
 * @constructor
 */
API = function (root) {
...
API.prototype = {
    ...
    createAppModule: function () {
        ...
        ncp(path.join(appPath, 'static'), this.root, function (err) {
            ...
            self.createAppTemps(self.root);

            self.createTempFiles(options.type, path.join(appPath, 'temp'), self.root);
        });
    },
    /**
     * Create empty directories and ignore file that npm does not handle
     * @param appPath
     */
    createAppTemps: function (appPath) {
        fs.rename(appPath + '/gitignore', appPath + '/.gitignore');
        fs.mkdir(appPath + '/assets/images');
        fs.mkdir(appPath + '/js/common');
        fs.mkdir(appPath + '/js/module');
    },
    /**
     *  JavaScript module
     */
    createJSModule: function () {
        var content = this.replaceTemplate('define([\n    "text!./assets/tpl/$name-tpl.html"\n], function ($lnameTpl) {\n});');
    ...
    getTemps: function (type) {
        ...
        switch (type) {
            case 'app':
                temps = ['bower.json', 'Gruntfile.js', 'package.json'];
                break;
            case 'm':
